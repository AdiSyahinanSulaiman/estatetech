  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.auto_awesome, color: Colors.amber, size: 20), onPressed: _loadAI),
          Padding(padding: const EdgeInsets.only(right: 15), child: GlobalUserDP(radius: 16, onTap: widget.onDPClick)),
        ],
      ),
      body: Stack(
        children: [
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasError) return const Center(child: Text("Error loading properties", style: TextStyle(color: Colors.white)));
              ...
              for (var doc in snapshot.data!.docs) {
                try {
                  props.add(Property.fromMap(doc.data() as Map<String, dynamic>, doc.id));
                } catch (e) { print("Skipping bad property data: $e"); }
              }

              // Apply Safeguarded AI Sort
              List<Property> sortedList = _ai.rankFeed(props, _userVector);

              if (sortedList.isEmpty) return const Center(child: Text("No listings available", style: TextStyle(color: Colors.white)));
              ...
              return PageView.builder(
                ...
                itemBuilder: (context, i) {
                  ...
                  return Stack(children: [
                    SizedBox.expand(child: Image.network(item.imageUrl, fit: BoxFit.cover, errorBuilder: (c, e, s) => Container(color: Colors.grey))),
                    ...
                    // Landlord Info on Left (DP Fixed)
                    Positioned(top: 120, left: 20, child: Row(children: [
                      CircleAvatar(
                        radius: 18,
                        backgroundColor: Colors.white24,
                        backgroundImage: item.sellerPhoto.isNotEmpty ? NetworkImage(item.sellerPhoto) : null,
                        child: item.sellerPhoto.isEmpty ? Text(item.sellerName[0], style: const TextStyle(color: Colors.white, fontSize: 12)) : null,
                      ),
          ...
          if (widget.isUploading)
            Positioned(top: 0, left: 0, right: 0, child: Container(color: Colors.black.withOpacity(0.8), padding: const EdgeInsets.only(top: 50, bottom: 10), child: const Column(children: [
              LinearProgressIndicator(backgroundColor: Colors.transparent, color: Colors.blueAccent),
              SizedBox(height: 5),
              Text("Publishing listing...", style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
            ]))),
        ],
      ),
    );
