...
class _DetailsScreenState extends State<DetailsScreen> {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Stack(children: [
            SizedBox(height: 400, child: PageView.builder(itemCount: images.length, onPageChanged: (i) => setState(() => _activeImageIndex = i), itemBuilder: (context, index) => Image.network(images[index], fit: BoxFit.cover))),
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              InkWell(
                ...
                child: Row(children: [
                  CircleAvatar(radius: 20, backgroundImage: NetworkImage(widget.property.sellerPhoto.isNotEmpty ? widget.property.sellerPhoto : "https://ui-avatars.com/api/?name=${widget.property.sellerName}")),
                  const SizedBox(width: 12),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(widget.property.sellerName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)), const Text("View Landlord Profile", style: TextStyle(color: Colors.blue, fontSize: 12))]),
                ]),
              ),
              const SizedBox(height: 30),
              ...
              Container(height: 200, width: double.infinity, decoration: BoxDecoration(borderRadius: BorderRadius.circular(15), border: Border.all(color: Colors.grey.withOpacity(0.2))), child: ClipRRect(borderRadius: BorderRadius.circular(15), child: GoogleMap(initialCameraPosition: CameraPosition(target: LatLng(widget.property.latitude, widget.property.longitude), zoom: 15), markers: { Marker(markerId: const MarkerId("loc"), position: LatLng(widget.property.latitude, widget.property.longitude)) }, liteModeEnabled: true))),
              ...
              StreamBuilder<QuerySnapshot>(
                  stream: FirebaseFirestore.instance.collection('bookings')
                      ...
                      .where('tenantId', isEqualTo: myId)
                      .snapshots(),
                  builder: (context, snap) {
                    var bookings = snap.data?.docs ?? [];
                    var activeBooking = bookings.isEmpty ? null : bookings.first.data() as Map<String, dynamic>;
                    String status = activeBooking?['status'] ?? "none";

                    return Column(
                      children: [
                        if (status == "Approved") ...[
                          _confirmedBanner(activeBooking!['date'], activeBooking['time']),
                        ...
                        Row(children: [
                          ...
                          if (status != "Approved") ...[
                            ...
                            Expanded(child: OutlinedButton(
                                onPressed: status == "Pending" || status == "Rescheduled" ? null : _bookViewing,
                                style: OutlinedButton.styleFrom(minimumSize: const Size(0, 55), side: BorderSide(color: status == "Pending" ? Colors.grey : navyBlue), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                                child: Text(status == "Pending" ? "Request Pending" : status == "Rescheduled" ? "Rescheduled" : "Book Viewing", style: TextStyle(color: status == "none" ? navyBlue : Colors.grey, fontWeight: FontWeight.bold)))),
                          ]
                        ]),
                      ],
                    );
  ...
  Widget _confirmedBanner(String date, String? time) {
    return Container(
      ...
      child: Column(children: [
        ...
        const Text("Viewing Confirmed!", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.green)),
        Text("Scheduled for $date at ${time ?? 'TBD'}", style: const TextStyle(color: Colors.green, fontSize: 13)),
      ]),
    );
  }
